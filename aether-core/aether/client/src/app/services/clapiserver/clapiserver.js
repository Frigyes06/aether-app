...
exports.StartClientAPIServer = void 0;
...
function SendNewView(req, callback) {
    var r = req.request.toObject();
    vuexStore.dispatch('setNewView', r);
    var resp = new messages.NewViewResponse();
    callback(null, resp);
}
...
function SendOnboardCompleteStatus(req, callback) {
    ...
    */
    var versionAndBuild = window.electronAPI.get_version();
    ...
    feapiconsumer.SendClientVersion(versionAndBuild, function (resp) {
        ...
        if (r.onboardcomplete) {
            ...
            */
            if (firstEverOpen) {
                var metrics_1 = window.electronAPI.metrics()();
                metrics_1.SendRaw('App first-ever opened');
                return;
            }
            if (firstOpenAfterSuccessfulUpdate) {
                var metrics_2 = window.electronAPI.metrics()();
...
function SendAlwaysShowNSFWListStatus(req, callback) {
    console.log('nsfw list status arrived.');
    var r = req.request.toObject();
    vuexStore.dispatch('setAlwaysShowNSFWListStatus', r.alwaysshownsfwlist);
    var resp = new messages.AlwaysShowNSFWListStatusResponse();
    callback(null, resp);
}
...
function SendSFWListDisabledStatus(req, callback) {
