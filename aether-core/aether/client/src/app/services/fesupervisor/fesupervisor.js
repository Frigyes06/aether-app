...
var StartFrontendDaemon = function (clientAPIServerPort) {
    ...
    if (!isDev) {
        // In production
        var fepath = path.join(__dirname, '../../../../../app', 'aether-frontend-' + getOsAndArch());
        console.log('fepath');
        console.log(fepath);
        child = spawn(fepath, [
            'run',
            '--isdev=false',
            "--clientip=".concat(clientAPIServerIP),
            "--clientport=".concat(clientAPIServerPort),
        ], {
    ...
    */
    ...
    globals.FrontendDaemon.on('exit', function (code, signal) {
        ...
        console.log('Frontend process exited with ' + "code ".concat(code, " and signal ").concat(signal));
