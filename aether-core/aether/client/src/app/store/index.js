...
var actions = __assign(
  {
    ...
    */
    ...
    setAmbientStatus: function (context, ambientStatus) {
      ...
      for (
        var _i = 0, _a = context.state.ambientStatus.inflights.boardsList;
        _i < _a.length;
        _i++
      ) {
        var val = _a[_i]
    ...
    setAmbientLocalUserEntity: function (
      context,
    ...
    setAlwaysShowNSFWListStatus: function (context, alwaysShowNSFWList) {
      context.commit('SET_ALWAYS_SHOW_NSFW_LIST_STATUS', alwaysShowNSFWList)
    },
    setExternalContentAutoloadDisabledStatus: function (
      context,
    ...
    setSFWListDisabledStatus: function (context, sfwListDisabled) {
...
var mutations = __assign(
  {
    SET_SIDEBAR_STATE: function (state, sidebarOpen) {
  ...
  contentRelations.mutations,
  {
    ...
    */
    ...
    SET_NOTIFICATIONS: function (state, payload) {
      ...
      for (var _i = 0, _a = state.notifications; _i < _a.length; _i++) {
        ...
        if (!val.read) {
          ...
          for (
            var _b = 0, _c = state.alreadyRaisedOSNotifications;
            _b < _c.length;
            _b++
          ) {
            var v = _c[_b]
      ...
      if (unreads.length > 1) {
        notification = new Notification('New notifications', {
          body:
            'You have ' + unreads.length + ' unread notifications on Aether.',
        })
    ...
    SET_ALWAYS_SHOW_NSFW_LIST_STATUS: function (state, alwaysShowNSFWList) {
      state.alwaysShowNSFWList = alwaysShowNSFWList
      state.alwaysShowNSFWListArrived = true
    },
    SET_EXTERNAL_CONTENT_AUTOLOAD_DISABLED_STATUS: function (
      state,
      externalContentAutoloadDisabled
    ) {
    ...
    SET_SFW_LIST_DISABLED_STATUS: function (state, sfwListDisabled) {
      // If there's a change, apply the new whitelist.
      if (
